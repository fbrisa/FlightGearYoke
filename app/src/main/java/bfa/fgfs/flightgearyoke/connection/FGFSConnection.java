package bfa.fgfs.flightgearyoke.connection;
 ...
 *
 * <p>
 * This class currently uses the FlightGear telnet interface, though it may be
 * modified to use a different TCP/IP interface in the future. Client
 * applications can use this library to examine and modify internal FlightGear
 * properties.
 * </p>
 *
 * <p>
 * To start FlightGear with the telnet server activated, use a command like this
 ...
 *
 * <p>
 * All methods that communicate directly with FlightGear are synchronized, since
...
public class FGFSConnection {
     ...
     *
     * <p>
     * Create a new connection to a running FlightGear program. The program must
     ...
     *
     * <p>
     * This is the primitive method for all property lookup; everything comes in
     * as a string, and is only later converted by methods like
     * {@link #getDouble(String)}. As a result, if you need the value as a
     * string anyway, it makes sense to use this method directly rather than
     * forcing extra conversions.
     * </p>
     *
     * @param name
     *            The FlightGear property name to look up.
     * @return The property value as a string (non-existant properties return
     *         the empty string).
     * @exception IOException
     *                If there is an error communicating with FlightGear or if
    ...
    public synchronized ArrayList<String> getContentLines(InputStream is)
            throws IOException {
        byte[] inbuffer = new byte[4096];
        StringBuffer sb = new StringBuffer();
    ...
    public synchronized ArrayList<String> getAllProps(String path)
            throws IOException {
        out.println("ls " + path + '\r');
        ArrayList<String> lines = new ArrayList<String>();
        String line;
    ...
    HashMap<String, String> lastSet = new HashMap<String, String>();

    /**
     * Set the raw string value for a property.
     *
     * <p>
     * This is the primitive method for all property modification; everything
     * goes out as a string, after it has been converted by methods like
     * {@link #setDouble(String,double)}. As a result, if you have the
     * vsetFloatalue as a string already, it makes sense to use this method
     * directly rather than forcing extra conversions.
     * </p>
     *
     * @param name
    ...
    public synchronized void set(String name, String value) throws IOException {
        ...
        if (!lastSet.containsKey(name)) {
            ...
            if (!lastSet.get(name).equals(value)) {
                sendIt = true;
            }
        }
